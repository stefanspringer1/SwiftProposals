...
A strong argument in favour of the proposed change is that `forEach` loops are considered as bad practice by some people (see the discussion of alternative 2 below), and adding the proposed change to for-in loops makes it less “necessary” having to resort to `forEach` loops.
...
How the alternative of using `forEach` is seen depends if one considers `forEach` as a sensible alternative to for-in loops:

- If one does consider `forEach` as a sensible alernative to for-in loops: A for-in loop and `forEach` have different semantics, e.g. you might want to use a for-in loop to be able to stop the iteration (see the example in the motivation). It should then be possible to switch between those two loop options according to your needs without being “penalized” with a more complex formulation in one case.

...
The solution in this proposal follows this alternative solution, which is kind of a more general solution in comparison to the rejected one, allowing the same sequence term (which could contain a trailing question mark) as could be written in front of the dot of a method call. This means that when switching between the for-in loop and an application of `forEach`, this term would not have to be changed. The optionality is always clear from a question mark in the sequence term (for what we mean by “sequence term”, see the section “Proposed solution” below). While motivations described in the old, rejected proposal might still be valid, we try a fresh take in the motivation section below, as the proposed solution differs from the rejected one.

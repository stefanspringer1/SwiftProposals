...
This means that when switching between the for-in loop and an application of `forEach`, this term would not have to be changed. Note that – as discussed below – using `forEach` might be considered as bad practice, so being able to change to for-in loops with any hassle should be considered a good thing.
